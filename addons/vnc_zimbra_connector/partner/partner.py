...
class zimbra_partner(osv.osv_memory):
    ...
    def history_message(self, cr, uid, vals):
        ...
        for ref_id in ref_ids:
            for model in eval(ref_id).keys():
                for res_id in eval(ref_id).get(model, []):
...
class res_partner(osv.osv):
    ...
    def partner_sync_openerp(self,cr, uid, zuid=False, addbookid=False, zim_domain=[], context=None):
        # synchronize contact to zimbra
        datas = False
        ...
        if zsync_ids:
            ...
            domain = [('zcontact_id','=',False)] + zdomain
